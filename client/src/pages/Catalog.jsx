    ...
    <div className="min-h-screen bg-[#0a0a0c] text-zinc-300 font-sans flex flex-col">
      ...
      <nav className="h-16 border-b border-zinc-900 bg-[#0a0a0c]/80 backdrop-blur-md sticky top-0 z-50 flex items-center justify-between px-8 shrink-0">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            ...
            <Link
              to="/login"
              className="bg-white text-black hover:bg-zinc-200 text-sm font-bold py-2 px-5 rounded-full transition-colors"
            >
              Sign In
            </Link>
          )}
      ...
      <div className="pt-20 pb-12 px-8 text-center max-w-3xl mx-auto w-full shrink-0">
        ...
        <p className="text-lg text-zinc-400 mb-8">
          Join industry-leading masterclasses, developer conferences, and
          interactive live streams from around the globe.
        </p>
        ...
        {!isLoading && !error && events.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((event) => (
              <div
                key={event._id}
                className="bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden group hover:border-zinc-700 transition-all hover:shadow-xl hover:shadow-black/40 flex flex-col"
              >
      ...
      <CheckoutModal
        ...
        event={selectedEvent}
        onBookingSuccess={() => {
          alert("Ticket secured successfully!");
          setIsModalOpen(false);
        }}
      />
    </div>
